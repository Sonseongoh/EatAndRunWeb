...
type TextAnalyzeFormProps = {
  onDirtyChange?: (dirty: boolean) => void;
};

export function TextAnalyzeForm({ onDirtyChange }: TextAnalyzeFormProps) {
  const { t } = useLocale();
  ...
  useEffect(() => {
    onDirtyChange?.(Boolean(foodText.trim() || analyzeMutation.data));
  }, [analyzeMutation.data, foodText, onDirtyChange]);
    ...
    <>
      <div className="space-y-3">
        ...
        <textarea
          ...
          placeholder={t(
            "예) 닭가슴살 샐러드 + 고구마 1개",
            "e.g. Chicken salad + 1 sweet potato",
          )}
