...
## 4. Google 지도 키 운영 가이드

이 프로젝트는 지도 키를 2개로 분리하는 것을 권장합니다.

1. 브라우저용 키: `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY`
- 브라우저에 노출되는 키입니다.
- Application restrictions: `Websites`로 제한합니다.
- 허용 referrer 예시:
  - `https://eat-and-run-web.vercel.app/*`
  - `http://localhost:3000/*`
- API restrictions: `Maps JavaScript API`만 허용합니다.

2. 서버용 키: `GOOGLE_MAPS_API_KEY`
- 서버에서만 사용하는 키입니다.
- API restrictions: `Directions API`만 허용합니다.
- `NEXT_PUBLIC_` 접두사를 붙이면 안 됩니다.

분리 이유:
- 브라우저 노출 키와 서버 비노출 키를 분리하면, 키 유출 시 피해 범위를 줄일 수 있습니다.
...
5. 지도 키를 하나만 쓰거나 키 제한을 잘못 설정해 `/map` 기능이 실패
